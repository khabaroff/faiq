...
def _try_defuddle(url: str) -> tuple[str, str] | None:
    ...
    if not cmd:
        try:
            result = subprocess.run(["npx", "-y", "defuddle-cli", url], capture_output=True, text=True, timeout=60)
        ...
        return None
    try:
        result = subprocess.run([cmd, url], capture_output=True, text=True, timeout=60)
        if result.returncode == 0 and len(result.stdout.strip()) > 200:
